...
### User Story
作为 BU 数据管理员、数据标注工程师和审核工程师，我想在 `标注任务`、`标注工作台` 与 `标注审核` 中完成标签模板、任务创建、任务分配、标注提交、审核通过/驳回、Label Studio 状态检测和标注数据集发布，以便把 DATA 域从数据集/Pipeline 延伸到可审计、可复用的标注闭环。
...
- 不复制原型 JSX，不恢复旧已删除实现。
- 不覆盖 CAD、复杂视频/音频逐帧标注工具细节；本期以平台控制面与 adapter seam 为验收范围。
...
- 标注工作台完整画布复杂：F012 只实现控制面、样本队列、提交/审核和外部工具 seam，复杂图像/视频标注工具后续另行规划。
