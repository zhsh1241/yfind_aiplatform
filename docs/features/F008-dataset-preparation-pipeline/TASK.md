...
### Frontend
- Pages: 按用户要求重做 `frontend/src/pages/DatasetPage.tsx` 为独立数据准备流水线工作台；7 个阶段分别提供独立处理页，不沿用原数据资产列表页。
...
- [ ] **AC-06**: 前端以独立数据准备流水线工作台展示任务、7 阶段进度、质量结果、阻断提示和重跑操作；每个阶段必须有独立处理页展示输入、功能处理、质量门禁与产出，不沿用原数据资产列表页。
