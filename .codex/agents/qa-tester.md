...
> Codex role brief: Generated from ".agents/agents/qa-tester.md". Use `AGENTS.md` and Codex child-agent routing as the execution authority. Treat legacy verbs like `Read`, `Agent`, `SendMessage`, and `EnterWorktree` as workflow instructions, not CLI-native commands. Ignore stale frontmatter fields that reference older runtime-specific surfaces.
