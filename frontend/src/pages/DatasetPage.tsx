...
export default function DatasetPage({ source, preparationJobs, openDetail, onRerunPreparationJob }: Props) {
  ...
  return <section className="dataset-console-shell"><aside className="dataset-console-side"><div className="dataset-side-title">数据源管理</div><button className="dataset-side-link is-active" type="button">数据源接入</button><div className="dataset-side-title">数据集管理</div><button className="dataset-side-link" type="button">数据集管理</button><div className="dataset-side-title">标签管理</div><button className="dataset-side-link" type="button">标签管理</button><div className="dataset-side-title">数据加工</div><button className="dataset-side-link" type="button">预处理数据集</button><button className="dataset-side-link" type="button">标注数据集</button><button className="dataset-side-link" type="button">验证数据集</button></aside><main className="dataset-console-main"><Card className="dataset-main-card dataset-console-card" title={<Space wrap><span>数据源管理</span><Tag color={source === "backend" ? "green" : "gold"}>{source === "backend" ? "后端 API 已连接" : "本地 fallback"}</Tag><Tag color="blue">TASK-dataset-preparation-pipeline</Tag></Space>}>
    <Form layout="inline" className="dataset-console-form"><Form.Item label="数据源类型"><Select className="dataset-console-select" defaultValue="全部类型" options={[{ value: "全部类型" }, { value: "Image" }, { value: "Text" }, { value: "Audio" }]} /></Form.Item><Form.Item label="数据源名称"><Input className="dataset-console-input" placeholder="请输入数据源名称" /></Form.Item><Form.Item label="共享状态"><Select className="dataset-console-select" defaultValue="请选择共享状态" options={[{ value: "请选择共享状态" }, { value: "私有" }, { value: "团队共享" }]} /></Form.Item><Form.Item><Button type="primary">查询</Button></Form.Item><Form.Item><Button>重置</Button></Form.Item></Form>
    <Space wrap className="dataset-console-actions"><Button type="primary">+ 新建数据源</Button>{firstJob && <Button danger={firstJob.blocked} onClick={() => onRerunPreparationJob(firstJob.jobId)}>人工修正后重跑</Button>}<Button onClick={() => openDetail("训练加载配置", "数据准备最终输出可训练数据集快照与 DataLoader 配置，不在本页提交训练任务。")}>查看加载配置</Button></Space>
    <Table className="dataset-console-table" size="small" pagination={{ pageSize: 5 }} dataSource={datasetRows} columns={[{ title: "数据源名称", dataIndex: "name", render: (value: string) => <Button type="link" className="dataset-row-link">{value}</Button> }, { title: "中文名称", dataIndex: "cnName" }, { title: "数据源类型", dataIndex: "type" }, { title: "状态", dataIndex: "status", render: (value: string) => <Tag color={value === "可训练" ? "green" : value === "处理中" ? "blue" : "gold"}>{value}</Tag> }, { title: "数据源版本", dataIndex: "version" }, { title: "驱动包名称", dataIndex: "driver" }, { title: "共享状态", dataIndex: "shareStatus" }, { title: "样本数", dataIndex: "count" }, { title: "操作", key: "action", render: () => <Space><Button type="link">公开</Button><Button type="link">编辑</Button><Button type="link" onClick={() => openDetail("更多操作", "删除、授权、发布、复制、上报元数据等动作按真实平台权限收敛。")}>更多</Button></Space> }]} />
  ...
  <Card className="dataset-main-card" title="七阶段功能处理页"><Paragraph className="tight-paragraph">这是按示例文档改造的数据部分页面：左侧为数据模块导航，上方为筛选区，中间为数据源表格，下方保留训练前数据准备的多阶段处理入口。</Paragraph>{preparationJobs.map((job) => <PreparationJobPanel key={job.jobId} job={job} openDetail={openDetail} onRerunPreparationJob={onRerunPreparationJob} onOpenStage={setSelectedStageKey} />)}</Card></main></section>;
}
