...
export const e2eUser = {
  ...
  roles: ['SUPER_ADMIN'],
  roleNames: ['超级管理员'],
  permissions: ['menu:dash', 'menu:hub', 'menu:usermgmt', 'menu:perm', 'menu:org', 'menu:sys', 'menu:resource', 'menu:datasrc', 'menu:ds', 'menu:portal', 'menu:lineage'],
  menuPermissions: ['dash', 'hub', 'usermgmt', 'perm', 'org', 'sys', 'resource', 'datasrc', 'ds', 'portal', 'lineage'],
  sessionVersion: 1,
};
...
export async function mockPlatformApis(page: Page) {
  ...
  await page.route('**/api/v1/data-source-sync-tasks/*/run', async (route) => { await route.fulfill({ json: { code: 0, message: 'success', traceId: 'e2e', timestamp: new Date().toISOString(), data: { ...syncTasks[0], status: 'FAILED', diagnosticMessage: 'TODO_CONFIRM_DATA_CONNECTOR_SCHEDULER' } } }); });
