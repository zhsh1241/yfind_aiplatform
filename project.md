...
完整基线见 `docs/architecture/01-technology-stack-baseline.md`；脚手架摘要同步在 `ai-scaffold.config.json` 的 `technologyStack`。这些是后续重建的默认技术选型。当前 backend/frontend/deploy 已恢复最小工程骨架，但业务实现仍必须按 feature 流程逐项落地。
...
完整路线图见 `docs/architecture/02-project-goals-and-roadmap.md`。当前重建策略是：
...
一期目标不以完整复刻全部原型页面为验收，而以“平台治理、数据资产、模型资产、边端下发、运营审计”形成生产可用闭环为验收。训练任务、开发环境、中心端在线推理、批量推理和灰度能力按 `docs/business/open-questions.md` 的已解决范围放入二期。

F003 `backend-foundation`、F004 `frontend-foundation`、F005 `deploy-foundation` 已作为工程骨架入口创建；后续业务 feature 从 `docs/features/NEXT_FEATURE_NUMBER.txt` 继续编号。
...
- `backend/`：已重建 Java 21 / Spring Boot 4.0.x 多模块最小工程骨架；旧实现仍不得作为事实来源。
- `frontend/`：已重建 React 19 / TypeScript 6 / Vite 8 / Ant Design 6 管理台最小工程骨架，并保留 25 个原型页面 key 的路由。
- `deploy/`：已重建 Docker、Compose、Helm 草案；外部环境参数继续使用 `TODO_CONFIRM_*`。
...
- `backend.enabled`: `true`，执行 Maven verify 门禁。
- `frontends[web].enabled`: `true`，执行 lint、Vitest 和 Vite build 门禁。
