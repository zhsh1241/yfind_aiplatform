...
public class DataManagementService {
    ...
    @Transactional public DataSourceSyncTaskResponse runSyncTask(PlatformPrincipal principal, String taskId) {
        ...
        String fileId = "FILE-" + randomHex(12).toUpperCase(Locale.ROOT); String objectKey = objectStorageService.objectKey(s.tenantId(), "dataset", "import", s.sourceType().toLowerCase(Locale.ROOT), taskId, fileId + "." + plan.extension()); String sha = sha256(plan.content()); String bucket = objectStorageService.datasetBucket(s.tenantId());
    ...
    @Transactional public DataStandardTaskResponse runDataStandardTask(PlatformPrincipal principal, String taskId) {
        ...
        String fileId = "FILE-" + randomHex(12).toUpperCase(Locale.ROOT), sha = sha256(taskId + "|" + src.datasetId() + "|" + after), objectKey = objectStorageService.objectKey(src.tenantId(), "dataset", "standardized", taskId, fileId + ".json"), standardizedBucket = objectStorageService.datasetBucket(src.tenantId());
    ...
    private void processSingleUploadFile(PlatformPrincipal principal, DatasetUploadSessionRecord session, String fileName, String contentType, byte[] bytes, OffsetDateTime now) {
        ...
        String objectKey = objectStorageService.objectKey(session.tenantId(), "dataset", "local-upload", session.sessionId(), sanitizeFileName(fileName));
