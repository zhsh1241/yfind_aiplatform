...
INSERT INTO platform_config_definition (config_key, group_name, display_name, value_type, default_value, sensitive, scope_allowed, validation_rule, status, created_at) VALUES
    ...
    ('storage.prefix', 'storage', '对象 Key 前缀', 'STRING', 'TODO_CONFIRM_STORAGE_PREFIX', FALSE, 'GLOBAL,BU', 'pathPrefix', 'ACTIVE', CURRENT_TIMESTAMP),
    ('storage.public-endpoint', 'storage', '对象存储公网 Endpoint', 'STRING', 'TODO_CONFIRM_MINIO_PUBLIC_ENDPOINT', TRUE, 'GLOBAL', 'todoConfirm', 'ACTIVE', CURRENT_TIMESTAMP),
    ('storage.presignedExpirySeconds', 'storage', '下载授权有效期', 'NUMBER', '900', FALSE, 'GLOBAL', 'min=60;max=3600', 'ACTIVE', CURRENT_TIMESTAMP),
